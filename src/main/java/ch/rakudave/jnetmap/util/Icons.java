...
public class Icons {
    ...
    private static String currentIconTheme = Settings.get("icon.theme", "Elementary");
    private static String currentDeviceTheme = Settings.get("device.theme", "cisco");
    ...
    private static Map<String, ImageIcon> loadImageIcons(String path) {
        ...
        try {
            ...
            for (String file : IO.listFiles(path, false)) {
                result.put(file.replace(".png", ""), loadImageIcon(path, file));
            }
        } catch (Exception e) {
    ...
    public static ImageIcon loadImageIcon(String path, String icon) {
        try {
            ...
            if (userIcon.exists()) return new ImageIcon(userIcon.getAbsolutePath());
            else return new ImageIcon(IO.getResource(path + "/" + icon));
        } catch (Exception e) {
