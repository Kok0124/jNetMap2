...
cd ${0%/*}
cd ..
BUILD_TIME="2019-05-01 00:00:00"
...
chmod 0755 tmp/usr/bin/jnetmap tmp/DEBIAN/p*
fakeroot dpkg-deb -b ./tmp jNetMap-${version}.deb
mv jNetMap-${version}.deb ../dist/
rm -rf tmp

# build rpm package
echo "building rpm..."
mkdir tmp
cp ../dist/jNetMap-${version}.deb tmp/
fakeroot alien --to-rpm --scripts -k tmp/jNetMap-${version}.deb
mv *.rpm ../dist/jNetMap-${version}.rpm
rm -rf tmp
...
lintian ../dist/jNetMap-${version}.deb
